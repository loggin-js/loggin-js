...
const { Formatter } = require('./formaters');
...
class Notifier {
  constructor(options) {
    ...
    if (!this.options.formater) {
      this.options.formater = new Formatter('[{time.toLocaleString}] - {channel} - {user} - {severityStr} - {message} - {JSON.stringify(message)}');
    } else if (typeof this.options.formater === 'string') {
      this.options.formater = new Formatter(this.options.formater);
    }
  }
