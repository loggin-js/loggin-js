...
const { debugFN, debugAction, checkType } = require('./utils')

class Notifier {
  constructor(options) {
    if (options.severity && !(options.severity instanceof Severity.Severity)) {
      throw 'ERROR: "options.severity" should be an instance of Severity. at: options.severity = ' + options.severity;
    }
  ...
  setLevel(severity) {
    ...
    process.env['LOGGER_JS_LEVEL'] = this.options.severity;
    return this;
  }
  ...
  notify(log) {
    let output;
    if (this.options.formatter) output = this.options.formatter(log);
    if (this.options.color) output = log.colored();
...
class FileNotifier extends Notifier {
  constructor(options) {
    ...
    if (this.options.pipes) {
      this.options.pipes.forEach((p, i) => {
        if (!(p instanceof Pipe))
          throw 'ERROR: "options.pipes" should be an array of Pipe objects. at: options.pipes[' + i + '] = ' + p
      })
  ...
  output(log, severity) {
    ...
    if (filepath) {
      ...
      if (!this.pipes.length) {
        throw 'If no "options.filepath" is passed you need to create a pipe. Check: "examples/piping.md"'
      }
      else {
  ...
  pipe(severity, filepath) {
    debugFN('pipe@FileNotifier', [severity, filepath]);
...
class Pipe {
  ...
  englobes(severity) {
    ...
    // For example the debug Severity should englobe any other severity.
      ...
      for (let i = 0; i < this.severity.englobes.length; i++) {
        const sev = this.severity.englobes[i];
