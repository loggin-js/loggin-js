...
class Notifier {
  constructor(options) {
    ...
    if (!this.options.formatter) {
      this.options.formatter = new Formatter('[{time.toLocaleString}] - {channel} - {user} - {severityStr} - {message} - {JSON.stringify(data)}');
...
class FileNotifier extends Notifier {
  ...
  output(log, severity) {
    ...
    let path_ = path.dirname(filepath); //filepath.substring(0, filepath.lastIndexOf('/'));
    console.log("Check exists: ", path_);
