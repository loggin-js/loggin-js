...
class Logger {
   ...
   * @param {string} name 
   */
  profile(name, opts = { colored: true }) {
    if (name in this._profiles) {
      ...
      this._profiles[name] = {
        startTime: Date.now(),
          ...
          let formatted;
          if (opts.colored) {
            formatted = log.colored({ template: '{message}' });
          } else {
            formatted = log.format({ template: '{message}' });
          }
...
class LoggerV2 {
  constructor(options, ...notifiers) {
    ...
    if (this._notifiers.length === 0) {
      this._notifiers = [
        new Notifiers.ConsoleNotifier(options)
      ];
    }
  ...
  debug(message, data = null, opts = {}) {
    this.log(message, data, { level: Severity.DEBUG, ...opts });
  ...
  warning(message, data = null, opts = {}) {
    this.log(message, data, { level: Severity.WARNING, ...opts });
  ...
  alert(message, data = null, opts = {}) {
    this.log(message, data, { level: Severity.ALERT, ...opts });
  ...
  emergency(message, data = null, opts = {}) {
    this.log(message, data, { level: Severity.EMERGENCY, ...opts });
  ...
  critical(message, data = null, opts = {}) {
    this.log(message, data, { level: Severity.CRITICAL, ...opts });
  ...
  error(message, data = null, opts = {}) {
    this.log(message, data, { level: Severity.ERROR, ...opts });
  ...
  notice(message, data = null, opts = {}) {
    this.log(message, data, { level: Severity.NOTICE, ...opts });
  ...
  info(message, data = null, opts = {}) {
    this.log(message, data, { level: Severity.INFO, ...opts });
