...
class Logger {
  constructor(notifier, options) {
    ...
    } else if (level.constructor.name === 'Severity') {
      this._level = level || Severity.DEBUG;
    } else {
      throw new Error('LoggerOptions "level" must be a <string> a <number> or a <Severity> instance');
    }
